...
const Reader = () => {
  ...
  // --- Early Callbacks (needed before useEffects) ---
  ...
  // Log when Reader component mounts and load article content
  useEffect(() => {
    logger.info("Reader component mounted, loading article content");
    loadArticle();
  }, [loadArticle]);
  ...
  // Direct scroll handler for progress bar and toolbar auto-hide
  useEffect(() => {
    ...
    const handleDirectScroll = () => {
      ...
      const scrollDelta = scrollPosition - lastScrollTopRef.current;
      if (Math.abs(scrollDelta) > 10) {
        if (scrollDelta > 0 && scrollPosition > 100) {
      ...
      // Show toolbar after user stops scrolling
