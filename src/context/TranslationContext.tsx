...
export const TranslationProvider: React.FC<TranslationProviderProps> = ({
  ...
  const [translatedElements, setTranslatedElements] = useState<
    Map<string, string>
  >(new Map());
  ...
  const contentSelector =
    "p, h1, h2, h3, h4, h5, h6, li, blockquote, figcaption, td, th, caption, div.content-text, pre, pre > code";
  ...
  const translateElement = useCallback(
    async (element: HTMLElement): Promise<void> => {
      if (
        ...
        element.classList.contains("readlite-translating")
      ) {
        return;
      }
      ...
      const existingTranslation = element.querySelector(
    ...
    `;
      ...
      const translationDiv = element.querySelector(
        ".readlite-translation",
      ) as HTMLElement;

      try {
        // Check cache first
        const cacheKey = originalText.substring(
          0,
          Math.min(100, originalText.length),
        );
   ...
   */
  const findAndTranslateContent = useCallback(
    async (containerElement: HTMLElement, text: string): Promise<void> => {
  ...
  const processTranslationBatch = useCallback(async () => {
    ...
    try {
      ...
      const translationPromises = currentBatch.map((element) => {
        ...
        return translateElement(element).catch((error) => {
      ...
      const progressNow = Math.min(
        99,
        Math.round(
          ((totalElements - translationQueue.current.length) / totalElements) *
            100,
        ),
      );
    ...
    if (
      !isTranslationCancelled.current &&
  ...
  const translateArticle = useCallback(
    async (containerElement: HTMLElement): Promise<void> => {
      if (isTranslating) {
