...
async function callLLMAPI(
  ...
  // Returns Response for stream, JSON object otherwise
  logger.debug(
    `Calling API with model: ${options.model}, stream: ${!!options.stream}`,
  );
  ...
  try {
    ...
    if (!response.ok) {
      ...
      if (response.status === 401) {
        logger.error(
          `API request failed with 401 Unauthorized. Body: ${errorBody}`,
        );
...
async function generateTextStreamInternal(
  ...
  try {
    ...
    const streamResponse = await callLLMAPI(messages, {
      model,
      maxTokens,
      temperature,
      stream: true,
    ...
    throw error instanceof Error
      ? error
      : new Error("Failed to generate streaming text.");
  }
}
...
async function processStreamInternal(
  ...
  const streamTimeoutId = setInterval(() => {
    ...
    if (timeSinceLastChunk > CHUNK_TIMEOUT_MS) {
      logger.warn(
        `Stream timed out - no chunks received for ${timeSinceLastChunk}ms`,
      );
