...
function sanitizeHtml(html: string): string {
  ...
  logger.info(
    `HTML content AFTER sanitization (first 500 chars):`,
...
function markArticleContainer(doc: Document): Element {
  ...
  for (const selector of containerSelectors) {
    ...
    if (elements.length > 0) {
      ...
      logger.info(
        `Found article container: ${container.tagName} with ${container.childNodes.length} child nodes`,
      );
...
function logDocumentTextSample(textNodes: Node[]): void {
  ...
  textNodes.slice(0, 10).forEach((node, i) => {
    ...
    fullDocText += content + " ";
    logger.info(
      `Node ${i}: "${content.substring(0, 100)}${content.length > 100 ? "..." : ""}"`,
    );
  });
}
...
function logHighlightDetails(highlight: StoredHighlight): void {
  ...
  logger.info(
    `Attempting to restore highlight: ${highlight.id} - "${highlight.text.substring(0, 30)}..."`,
  );
}
 ...
 */
function applySelectorBasedHighlight(
  doc: Document,
  highlight: StoredHighlight,
): boolean {
  ...
  if (success) {
    logger.info(
      `Successfully restored complex highlight ${highlight.id} using selector approach`,
    );
    return true;
  } else {
 ...
 * @param highlight The highlight to apply.
 */
async function applyNodeBasedHighlight(
  doc: Document,
  highlight: StoredHighlight,
): Promise<void> {
  ...
  if (matchingNodes.length === 0) {
    logger.warn(
      `No matching nodes found for highlight ${highlight.id}: "${highlight.text.substring(0, 20)}..."`,
    );
    return;
  }

  logger.info(
    `Found ${matchingNodes.length} potential nodes for highlight ${highlight.id}`,
  );
...
function tryApplyHighlightToNode(
  ...
  try {
    ...
    logger.info(
      `Searching in text node content: "${content.substring(0, 50)}${content.length > 50 ? "..." : ""}"`,
    );
    logger.info(
      `Looking for: "${highlight.text.substring(0, 50)}${highlight.text.length > 50 ? "..." : ""}"`,
    );
    ...
    // Find the text index
    const textIndex = findTextPosition(normalizedContent, normalizedSearchText);

    if (textIndex !== -1) {
      return createAndApplyHighlight(
        doc,
        textNode,
        textIndex,
...
function createAndApplyHighlight(
  ...
  try {
    ...
    logger.info(
      `Created range from index ${textIndex} to ${textIndex + maxLength}`,
    );
    ...
    if (rangeText.length < 3 || rangeText.length < searchText.length * 0.3) {
      ...
      const improvedRange = improveTextRange(
        doc,
        textNode,
        textIndex,
        content,
    ...
    try {
      ...
      logger.info(
        `Applied highlight ${highlight.id} to text "${highlight.text.substring(0, 20)}..."`,
      );
      return true;
    } catch (e) {
      // Try alternate approach if surroundContents fails
      return applyAlternateHighlightMethod(
        doc,
        range,
        highlightSpan,
        highlight,
...
function improveTextRange(
  ...
  try {
    ...
    const expandedEnd = Math.min(
      content.length,
      textIndex + searchText.length + 10,
    );
    ...
    for (let i = 0; i < 10; i++) {
      ...
      if (portionIndex !== -1) {
        ...
        bestMatchLength = Math.min(
          searchText.length,
          content.length - bestMatchStart,
        );
        logger.info(
          `Found better match at position ${bestMatchStart} using portion "${testPortion}"`,
        );
...
function applyAlternateHighlightMethod(
  ...
  logger.warn(
    `Failed to apply highlight ${highlight.id} with surroundContents, trying alternate method`,
  );
  ...
  try {
    ...
    logger.warn(
      `Alternative highlighting method also failed for ${highlight.id}:`,
      e2,
    );
  ...
  for (const selector of DATE_SELECTORS) {
    ...
    if (dateString) {
      try {
        ...
        logger.warn(
          `Could not parse date string "${dateString}" from selector "${selector}"`,
        );
      }
    }
  }
