...
export class AIClient {
  ...
  async generateTextStream(
    ...
    try {
      logger.info(
        `Starting generateTextStream with ${messages.length} messages`,
      );
      ...
      logger.info(
        `User prompt: "${lastUserMessage.content.substring(0, 50)}..."`,
      );

      // Use the existing llmClient's generateTextStream method
      await llmClient.generateTextStream(lastUserMessage.content, onChunk, {
