...
const Reader = () => {
  ...
  const {
    translateArticle: translateWithContext,
    isTranslating: isTranslationActive,
    translationProgress: translationProgressValue,
    ...
    const handleSelectionMessage = (event: MessageEvent) => {
      if (event.data && event.data.type === "TEXT_SELECTED") {
        ...
        if (rect) {
          ...
          event.data &&
          event.data.type === "TRANSLATE_SELECTION_COMPLETE"
        ) {
          if (event.data.success && event.data.selectedText) {
            ...
            llmClient
              .generateTextStream(prompt, handleStreamChunk, {
  ...
  const handleDrag = useCallback((e: MouseEvent) => {
    ...
    requestAnimationFrame(() => {
      ...
      const newWidth = Math.min(
        85,
        Math.max(30, initialWidthRef.current + percentageDelta),
      );
    ...
    const collectContentWhileScrolling = () => {
      ...
      if (Math.abs(scrollContainer.scrollTop - lastPosition) < 1) {
        ...
        if (++isAtBottom >= 5) {
          ...
          window.postMessage({ type: "AUTO_SCROLL_COMPLETED" }, "*");
          logger.info(
            "Auto-scroll completed, possibly reached end of document",
          );
          return;
        }
      } else {
    ...
    (selectedText: string) => {
      try {
        ...
        if (
          ...
          logger.info(
            "Starting translation for selected text:",
  ...
  const handleTranslateArticle = async () => {
    try {
      ...
      if (isLoggedIn) {
        ...
        logger.info(
          "Global translation requires login, opening Agent interface",
        );
        if (!showAgent) {
          toggleAgent();
        }
