...
// Removed unused import: llmClient
...
const TRANSLATABLE_CONTENT_SELECTOR =
...
const CONTENT_ELEMENTS_SELECTOR =
  "p, li, blockquote, div:not(.code-lang-label):not(.readlite-byline)";
...
const ReaderContent = forwardRef<HTMLDivElement, ReaderContentProps>(
  ({ settings, article, detectedLanguage, error }, ref) => {
    ...
    const readerColors = useMemo(
      () => getReaderColors(),
      [getReaderColors, theme],
    );
    const isCJKLanguage = useMemo(
      () => CJK_LANGUAGES.includes(detectedLanguage),
      [detectedLanguage],
    );
    ...
    const textAlignClass = useMemo(
      () => getTextAlignClass(settings.textAlign),
      [settings.textAlign],
    );
    ...
    const applyLinkStyles = (contentElement: HTMLElement, colors: any) => {
      ...
      const mouseLeaveListeners: Array<{
        element: HTMLElement;
        listener: EventListener;
      }> = [];
    ...
    const addContentIds = (contentElement: HTMLElement) => {
      ...
      logger.info(
        `Added IDs to ${contentElements.length} content elements for translation`,
      );
    };
      ...
      try {
        ...
        const highlightData = selection.highlightElement
          ? {
              ...
              color: selection.highlightElement.getAttribute(
