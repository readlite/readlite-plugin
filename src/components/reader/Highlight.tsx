...
export const Highlights: React.FC<Props> = ({ rootRef }) => {
  const { selection, applyHighlight, removeHighlight } = useTextSelection(rootRef);
  ...
  React.useEffect(() => {
    const handler = (e: MessageEvent) => {
      ...
      if (d.type === "REMOVE_HIGHLIGHT") {
        const el = rootRef.current?.querySelector(`[data-highlight-id="${d.highlightId}"]`);
        if (el) removeHighlight(el as HTMLElement);
      }
      if (d.type === "COPY_SELECTION" && selection.text) navigator.clipboard.writeText(selection.text);
    };
