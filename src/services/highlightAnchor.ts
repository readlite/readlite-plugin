...
export class HighlightAnchor {
  ...
  createAnchorData(
    ...
    StoredHighlight,
    "textBefore" | "textAfter" | "domPath" | "nodeIndex"
  > {
    try {
      ...
      if (textIndex === -1 && normalizedText.length > 10) {
        ...
        const firstPortion = normalizedText.substring(
          0,
          Math.min(normalizedText.length, 20),
        );
        textIndex = normalizedContent.indexOf(firstPortion);

        if (textIndex !== -1) {
          logger.info(
            `Using partial match for text "${normalizedText.substring(0, 20)}..."`,
          );
        } else {
          ...
          for (let i = 0; i < Math.min(normalizedText.length, 10); i++) {
            ...
            if (charPattern.length >= 3) {
              ...
              if (textIndex !== -1) {
                logger.info(
                  `Found character pattern match for "${charPattern}" in "${normalizedText.substring(0, 20)}..."`,
                );
                break;
              }
            }
      ...
      if (textIndex === -1) {
        logger.warn(
          `Text "${text.substring(0, 20)}..." not found in node content, using fallback positioning`,
        );
      ...
      const textBefore = normalizedContent.substring(
        Math.max(0, textIndex - contextLength),
        textIndex,
      );
      const textAfter = normalizedContent.substring(
        textIndex +
          Math.min(normalizedText.length, normalizedContent.length - textIndex),
        Math.min(
          normalizedContent.length,
          textIndex + normalizedText.length + contextLength,
        ),
      );
  ...
  createSelectorData(
    ...
    StoredHighlight,
    "textBefore" | "textAfter" | "domPath" | "nodeIndex" | "start" | "end"
  > & { exact: string } {
    try {
      ...
      const textAfter = fullText.slice(
        ...
        Math.min(fullText.length, end + context),
      );

      // Also add traditional DOM path for backward compatibility
      const domPath = this.getDomPath(
      ...
      logger.info(
        `Created selector data for text "${exact.substring(0, 20)}..."`,
      );
      logger.info(`Position selector: start=${start}, end=${end}`);
      logger.info(
        `Quote selector: prefix="${textBefore}", suffix="${textAfter}"`,
      );
  ...
  applyHighlightWithSelector(
    ...
    try {
      ...
      if (idx === -1 && selector.textBefore && selector.textAfter) {
        try {
          ...
          if (match) {
            ...
            logger.info(
              `Second match attempt: found with context at index ${idx}`,
            );
          }
        } catch (e) {
      ...
      if (idx === -1) {
        ...
        for (let i = 0; i < fullText.length - 5; i++) {
          const chunk = fullText.substring(
            i,
            i + Math.min(50, fullText.length - i),
          );
          const exactChunk = selector.exact.substring(
            0,
            Math.min(50, selector.exact.length),
          );
        ...
        if (bestMatch !== -1) {
          idx = bestMatch;
          logger.info(
            `Third match attempt: found with fuzzy matching at index ${idx} (score: ${bestScore.toFixed(2)})`,
          );
        }
      }
      ...
      if (idx === -1 && selector.start >= 0 && selector.end > selector.start) {
        idx = selector.start;
        logger.info(
          `Fourth match attempt: using position selector at index ${idx}`,
        );
      }
      ...
      if (idx === -1) {
        logger.warn(
          `Failed to find text "${selector.exact.substring(0, 30)}..." in content`,
        );
        return false;
      }
      ...
      if (startNode.data.length >= startOffset + exactLength) {
        ...
        let remainingLength =
          exactLength - (startNode.data.length - startOffset);
      ...
      if (startNode !== endNode) {
        ...
        textsInRange.forEach((node, i) => {
          try {
            ...
            try {
              ...
              logger.warn(
                `Failed to surround node ${i} with highlight, trying alternative:`,
                e,
              );
        ...
        logger.info(
          `Applied complex highlight with ${textsInRange.length} parts`,
        );
        return true;
      } else {
        ...
        try {
          ...
          logger.warn(
            "Failed to surround with highlight, trying alternative:",
            e,
          );
          ...
          try {
            ...
            const selectedText = startNode.data.substring(
              startOffset,
              endOffset,
            );
  ...
  findAnchorNodes(document: Document, highlight: StoredHighlight): Node[] {
    try {
      ...
      logger.info(
        `Looking for highlight text: "${highlight.text.substring(0, 50)}${highlight.text.length > 50 ? "..." : ""}"`,
      );
      ...
      const normalizedHighlightText = highlight.text
        ...
        .trim();
      logger.info(
        `Normalized highlight text: "${normalizedHighlightText.substring(0, 50)}${normalizedHighlightText.length > 50 ? "..." : ""}"`,
      );

      // If we have the DOM path, try to use it first
      if (
        highlight.domPath &&
        Array.isArray(highlight.domPath) &&
        highlight.domPath.length > 0
      ) {
        try {
          const elementByPath = this.getElementByDomPath(
            document,
            highlight.domPath,
          );
          ...
          if (elementByPath) {
            logger.info(
              `Found element by DOM path: ${highlight.domPath.join(" > ")}`,
            );
            ...
            if (textNodes.length > 0) {
              logger.info(
                `Found ${textNodes.length} text nodes in the target element`,
              );

              // If nodeIndex is available, try to get the specific text node
              if (
                highlight.nodeIndex !== undefined &&
                textNodes[highlight.nodeIndex]
              ) {
                const candidateNode = textNodes[highlight.nodeIndex];
                const candidateText = candidateNode.textContent || "";
                logger.info(
                  `Found node at index ${highlight.nodeIndex}: "${candidateText.substring(0, 50)}${candidateText.length > 50 ? "..." : ""}"`,
                );
                ...
                for (const node of textNodes) {
                  ...
                  logger.info(
                    `Comparing with node text: "${normalizedContent.substring(0, 50)}${normalizedContent.length > 50 ? "..." : ""}"`,
                  );
                  ...
                  // For CJK text, also try partial matching
                    ...
                    const firstPortion = normalizedHighlightText.substring(
                      0,
                      Math.min(normalizedHighlightText.length, 20),
                    );
                    if (normalizedContent.includes(firstPortion)) {
                      logger.info(
                        `✅ Found partial match with first portion: "${firstPortion}"`,
                      );
                      matchingNodes.push(node);
                    } else {
                      ...
                      for (
                        let i = 0;
                        i < Math.min(normalizedHighlightText.length, 20);
                        i += 5
                      ) {
                        const charPattern = normalizedHighlightText.substring(
                          i,
                          i + 10,
                        );
                        if (
                          charPattern.length >= 3 &&
                          normalizedContent.includes(charPattern)
                        ) {
                          logger.info(
            ...
            logger.info(
              `Could not find element by DOM path: ${highlight.domPath.join(" > ")}`,
            );
          }
        } catch (e) {
        ...
        logger.info(
          "No valid DOM path in highlight data, will use text-based search only",
        );
      }
      ...
      if (matchingNodes.length === 0) {
        ...
        if (highlight.textBefore || highlight.textAfter) {
          ...
          logger.info(
            `Searching through ${allTextNodes.length} text nodes in document body using context`,
          );
          ...
          for (const node of allTextNodes) {
            ...
            if (normalizedContent.includes(normalizedHighlightText)) {
              logger.info(
                `✅ Found text in node: "${content.substring(0, 30)}..."`,
              );
              matchingNodes.push(node);
            } else if (
              highlight.textBefore &&
              normalizedContent.includes(highlight.textBefore)
            ) {
              logger.info(
                `✅ Found text by 'textBefore' context: "${highlight.textBefore}"`,
              );
              matchingNodes.push(node);
            } else if (
              highlight.textAfter &&
              normalizedContent.includes(highlight.textAfter)
            ) {
              logger.info(
                `✅ Found text by 'textAfter' context: "${highlight.textAfter}"`,
              );
              matchingNodes.push(node);
            }
            // For CJK text, try additional matching strategies
              ...
              const middleStart = Math.max(
                0,
                Math.floor(normalizedHighlightText.length / 2) - 10,
              );
              const middlePortion = normalizedHighlightText.substring(
                middleStart,
                middleStart + 20,
              );
              if (
                middlePortion.length >= 10 &&
                normalizedContent.includes(middlePortion)
              ) {
                logger.info(
                  `✅ Found partial match with middle portion: "${middlePortion}"`,
                );
                matchingNodes.push(node);
                continue;
              }
        ...
        // If we couldn't find any nodes with context, try a broader approach
          ...
          const allTextNodes = this.getAllTextNodes(document.body);
          logger.info(
            `Searching through ${allTextNodes.length} text nodes with full text search`,
          );
          ...
          for (const node of allTextNodes) {
            ...
            if (normalizedContent.includes(normalizedHighlightText)) {
              logger.info(
                `✅ Found exact match in node: "${content.substring(0, 30)}..."`,
              );
              matchingNodes.push(node);
            }
            // Try partial matching for longer texts (common in CJK languages)
              ...
              if (
                lastPortion.length >= 10 &&
                normalizedContent.includes(lastPortion)
              ) {
                logger.info(
                  `✅ Found partial match with last portion: "${lastPortion}"`,
                );
                matchingNodes.push(node);
                continue;
              }
              ...
              for (
                let i = 0;
                i < normalizedHighlightText.length - chunkSize;
                i += 4
              ) {
                // Overlap by 4 chars
                const charPattern = normalizedHighlightText.substring(
      ...
      if (matchingNodes.length > 0) {
        logger.info(
          `Found ${matchingNodes.length} potential nodes for text "${highlight.text.substring(0, 20)}..."`,
        );
      } else {
        logger.warn(
          `No nodes found for text "${highlight.text.substring(0, 20)}..."`,
        );
      }
  ...
  private getDomPath(element: Element): string[] {
    try {
      ...
      if (articleContainer) {
        logger.info(
          `Found article container for relative path: ${articleContainer.tagName}`,
        );
    ...
    try {
        ...
        if (match && match[1]) {
          ...
          const findArticleContainer = (): Element | null => {
            ...
            const containers = doc.querySelectorAll(
              'article, [role="main"], .article, .content, .post-content, .readlite-reader-container',
            );
          ...
          if (articleContainer) {
            ...
            if (relativePath) {
              ...
              if (pathSegments.length > 0) {
                ...
                const lastSegments = pathSegments.slice(
                  -Math.min(2, pathSegments.length),
                );

                try {
                  const query = lastSegments.join(" > ");
                  logger.info(
                    `Trying relative path with last segments: ${query}`,
                  );
                  const elements = articleContainer.querySelectorAll(query);
                  if (elements.length > 0) {
                    logger.info(
                      `Found ${elements.length} elements with partial relative path`,
                    );
                ...
                if (tagOnlySegments.length > 0) {
                  try {
                    ...
                    logger.info(
                      `Trying relative path with tag names only: ${tagQuery}`,
                    );
                    const elements =
                      articleContainer.querySelectorAll(tagQuery);
                    if (elements.length > 0) {
                      logger.info(
                        `Found ${elements.length} elements with tag-based relative path`,
                      );
                      return elements[0] as Element;
                    }
                  } catch (e) {
      ...
      if (currentPath.length > 0) {
        try {
          ...
          logger.info(
            `Trying partial path (most specific segments): ${pathQuery}`,
          );
