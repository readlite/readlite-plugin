...
interface LlmStreamRequestMessage {
  type: "LLM_STREAM_REQUEST";
  data: LlmStreamRequestData;
}
interface LlmApiRequestMessage {
  type: "LLM_API_REQUEST";
  data: LlmApiRequestData;
}
    ...
    switch (message.type) {
      ...
      case "GET_MODELS_REQUEST":
        handleGetModelsRequest(
          message as GetModelsRequestMessage,
          sendResponse,
        );
        return true; // Async response possible
      default:
...
function handleLlmApiRequest(
  ...
  if (
    data.method === "generateTextStream" &&
    typeof moduleToCheck?.generateTextStream === "function"
  ) {
    // Pass the actual function reference
    handleStreamingRequest(
      moduleToCheck.generateTextStream,
      data.params,
      sender,
      sendResponse,
    );
    ...
    handleRegularApiRequest(
      data.method,
      moduleToCheck[data.method],
      data.params,
      sendResponse,
    );
  } else {
    llmApiLogger.error(
      `Method "${data.method}" not found or not a function in LLM module.`,
    );
    sendResponse({
      success: false,
      error: `Method ${data.method} not found in LLM API`,
    });
  }
}
  ...
  try {
    ...
    if (typeof moduleToCheck?.generateTextStream !== "function") {
      ...
      port.postMessage({
        ...
        error: "Stream method not available",
      });
      return;
    }

    streamLogger.info(
      `Setting up stream for prompt: "${prompt?.substring(0, 30)}..."`,
    );
    ...
    const timeoutId = setTimeout(() => {
      if (!streamCompleted) {
        streamLogger.warn(
          `Port stream timeout triggered after ${PORT_STREAM_TIMEOUT_MS / 1000} seconds.`,
        );
        ...
        try {
          ...
          streamLogger.error(
            `Error sending timeout message: ${e instanceof Error ? e.message : String(e)}`,
          );
        }
      }
    ...
    const streamHandler = (chunk: string) => {
      try {
        ...
        streamLogger.error(
          `Error sending chunk via port: ${error instanceof Error ? error.message : String(error)}`,
        );
      }
    };
    ...
    moduleToCheck
      .generateTextStream(prompt, streamHandler, options)
      .then(() => {
    ...
    streamLogger.error(
      `Error in stream setup: ${error instanceof Error ? error.message : String(error)}`,
    );
    port.postMessage({
      type: "LLM_STREAM_ERROR",
...
function handleStreamingRequest(
  ...
  try {
    ...
    streamLogger.info(
      `Setting up stream handler for prompt starting with: "${prompt?.substring(0, 30)}..."`,
    );
    ...
    const streamHandler = (chunk: string) => {
      try {
        ...
        if (!chrome.runtime?.id) {
          streamLogger.warn(
            `Extension context invalid, cannot send stream chunk.`,
          );
        ...
        chrome.runtime
          ...
          .catch((error) => {
            ...
            streamLogger.info(
              `Consider using port-based streaming for more reliable communication`,
            );
          });
      } catch (error) {
        streamLogger.error(
          `Error inside stream handler: ${error instanceof Error ? error.message : String(error)}`,
        );
      }
    };
    ...
    const timeoutId = setTimeout(() => {
      if (!streamCompleted) {
        ...
        if (localSendResponse) {
          localSendResponse({
            success: false,
            error: "Stream timed out after 120 seconds",
          });
    ...
    streamMethod(prompt, streamHandler, streamOptions)
      .then(() => {
        ...
        if (localSendResponse) {
          // Check if timeout already responded
          localSendResponse({ success: true });
        } else {
          streamLogger.info(
            `Stream completed, but response already sent by timeout.`,
          );
        }
      })
      .catch((error: Error) => {
        ...
        if (localSendResponse) {
          ...
          streamLogger.info(
            `Stream failed, but response already sent by timeout.`,
          );
        }
      });
  } catch (error) {
    streamLogger.error(
      `Error during stream setup: ${error instanceof Error ? error.message : String(error)}`,
    );
    streamLogger.error(
      `Error details: ${error instanceof Error ? error.message : String(error)}`,
    );
  ...
  try {
    apiMethod(...(params || []))
      ...
      .catch(async (error: Error) => {
        ...
        if (isAuthError) {
          llmApiLogger.warn(
            `Authentication error detected in "${method}", handling token expiry`,
          );
 ...
 */
function handleAuthStatusChanged(
  isAuthenticated: boolean,
  sendResponse: (response?: any) => void,
) {
      ...
      if (tab.id) {
        chrome.tabs
          ...
          .catch((error) => {
            ...
            mainLogger.debug(
              `Failed to send auth status to tab ${tab.id}: ${error instanceof Error ? error.message : String(error)}`,
            );
          });
      }
    }
...
async function fetchModelsInBackground(
  ...
  mainLogger.info(
    `Attempting to fetch models... (attempt ${retryCount + 1}/${maxRetries + 1})`,
  );
  try {
    ...
    if (token) {
      ...
      mainLogger.warn(
        `No auth token available, this may result in limited model access`,
      );
    }

    const response = await fetch("https://api.readlite.app/api/models", {
      headers,
    });
    ...
    if (!isAuthError && retryCount < maxRetries) {
      mainLogger.info(
        `Will retry in ${retryDelay}ms (attempt ${retryCount + 1}/${maxRetries})`,
      );
      return new Promise((resolve) => {
        setTimeout(() => {
          resolve(
            fetchModelsInBackground(retryCount + 1, maxRetries, retryDelay),
          );
        }, retryDelay);
      });
    }
 ...
 */
function handleGetModelsRequest(
  message: GetModelsRequestMessage,
  sendResponse: (response?: any) => void,
) {
  ...
  if (
    ...
    mainLogger.info(
      `Returning cached models (${availableModels.length} items)`,
    );
