...
const HIGHLIGHT_KEY = "local:readlite-highlights";
...
export class HighlightStorage {
  ...
  async saveHighlight(highlight: StoredHighlight): Promise<void> {
    try {
      ...
      await storage.setItem(HIGHLIGHT_KEY, highlights);
  ...
  async deleteHighlight(id: string): Promise<boolean> {
    try {
      ...
      await storage.setItem(HIGHLIGHT_KEY, filteredHighlights);
  ...
  async getAllHighlights(): Promise<StoredHighlight[]> {
    try {
      const data = await storage.getItem<StoredHighlight[]>(HIGHLIGHT_KEY);
      return data || [];
    } catch (error) {
